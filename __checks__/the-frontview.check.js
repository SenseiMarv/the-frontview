...
test('components', async ({ page }) => {
  ...
  await expect(
    page.getByRole('article').getByRole('link', { name: 'Favicon TailwindCSS' })
  ).toBeVisible();
  ...
  // Embedded content
  await expect(
    page.getByRole('article').getByRole('heading', { name: 'Embedded content' })
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole('article').getByRole('code').filter({ hasText: 'component' })
  ).toBeVisible();
  await expect(
...
test('privacy policy page', async ({ page }) => {
  ...
  await expect(
    page.getByText('This website is using Umami to collect tracking data.')
  ).toBeVisible();
});
